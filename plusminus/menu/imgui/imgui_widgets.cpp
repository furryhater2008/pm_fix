...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 9;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + 11 + label_size.x, (label_size.y > square_sz) ? label_size.y : square_sz));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (*v)
        it_anim->second.active_rect_alpha += 25 * (1.5f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.active_rect_alpha -= 25 * (1.5f - ImGui::GetIO().DeltaTime);
    ...
    window->DrawList->AddRectFilledMultiColor(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(43, 43, 43), ImColor(43, 43, 43), ImColor(0.1f, 0.1f, 0.1f), ImColor(0.1f, 0.1f, 0.1f));

    if (*v)
        window->DrawList->AddRectFilledMultiColor(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(112, 128, 144, it_anim->second.active_rect_alpha), ImColor(112, 128, 144, it_anim->second.active_rect_alpha), ImColor(128, 128, 128), ImColor(128, 128, 128));

    window->DrawList->AddRect(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(255, 255, 255, 45));

    RenderText({ total_bb.Max.x - label_size.x - 7, total_bb.Min.y - 2 }, label);
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = ImGui::InputTextEx(label, NULL, buf, IM_ARRAYSIZE(buf), bb.GetSize(), flags);
    if (init)
    {
...
static void ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
    ...
    ImGuiContext& g = *GImGui;
    if (g.ColorEditLastColor1 != ImGui::ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0)))
        return;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            ...
            if (g.ColorEditLastColor1 == ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0)))
                H = g.ColorEditLastHue;
            value_changed = value_changed_sv = true;
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            g.ColorEditLastColor1 = ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0));
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            col[0] = H;
...
void ChadRenderColorRectWithAlphaCheckerboard(ImDrawList* draw_list, ImVec2 p_min, ImVec2 p_max, ImU32 col, float grid_step, ImVec2 grid_off, float rounding, int rounding_corners_flags = -1) {
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF) {
        ImU32 col_bg1 = ImGui::GetColorU32(ImAlphaBlendColor(IM_COL32(204, 204, 204, 255), col));
        ImU32 col_bg2 = ImGui::GetColorU32(ImAlphaBlendColor(IM_COL32(128, 128, 128, 255), col));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
        ...
        ChadRenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight);
        ...
        if (col_source.w < 1.0f)
            ChadRenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
    ...
    if (pressed) {
        ImGui::OpenPopup((std::string(desc_id) + "##CustomColorPicker").c_str());
    }

    if (ImGui::BeginPopup((std::string(desc_id) + "##CustomColorPicker").c_str())) {
        ImGui::ColorPicker4((std::string(desc_id) + "##CustomColorPicker").c_str(), (float*)&col, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoSmallPreview);
